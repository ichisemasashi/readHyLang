...
*[https://autbor.com/mypets/](https://autbor.com/mypets/)*.
...
[range(len(][someList][))] technique
with a [for] loop to obtain the integer index of the items in
the list, you can call the [enumerate()] function instead. On
each iteration of the loop, [enumerate()] will return two
...
The [random] module has a couple functions that accept lists
for arguments. The [random.choice()] function will return a
...
variable itself. For example, after assigning [42] to the
...
[-], [\*], [/], and [%]
operators, described in [Table 4-1](#calibre_link-711).
...
ops}type="pagebreak"}The [+=] operator can also do string and
...
value. For example, if a list value were stored in [spam], you
would call the [index()] list method (which I'll explain
shortly) on that list like so: [spam.index('hello')]. The
...
List values have an [index()] method that can be passed a
...
To add new values to a list, use the [append()] and
[insert()] methods. Enter the following into the interactive
...
any index in the list. The first argument to [insert()] is the
...
spam.append('moose')] and [spam = spam.insert(1,
'chicken')]. Neither [append()] nor
[insert()] gives the new value of [spam] as its
return value. (In fact, the return value of [append()] and
[insert()] is [None], so you definitely wouldn't
...
Types](#calibre_link-176)" on [page
94](#calibre_link-712).

Methods belong to a single data type. The [append()] and
[insert()] methods are list methods and can be called only on
...
ops}type="pagebreak"}Like the [sort()] list method,
[reverse()] doesn't return a list. This is why you write
[spam.reverse()], instead of [spam =
spam.reverse()].
...
identical [elif] statements, you can create a single list that
...
[random.randint (0, len(messages) - 1)]. This produces a
random number to use for the index, regardless of the size of
[messages]. That is, you'll get a random number between
[0] and the value of [len(messages) - 1]. The
benefit of this approach is that you can easily add and remove strings
to the [messages] list without changing other lines of code.
...
[range()], and tuples (explained in the "[The Tuple Data
Type](#calibre_link-177)" on [page
96](#calibre_link-713)). Many of the things you can do with
lists can also be done with strings and other values of sequence types:
indexing; slicing; and using them with [for] loops, with
...
4-3](#calibre_link-715) depicts the seven changes made by the
...
*Figure 4-3: The [del] statement and the
[append()] method modify the same list value in place.*

Changing a value of a mutable data type (like what the [del]
statement and [append()] method do in the previous example)
...
but "[Passing References](#calibre_link-181)" on [page
100](#calibre_link-716) will explain the different behavior
...
Just like how [str(42)] will return ['42'], the
string representation of the integer [42], the functions
[list()] and [tuple()] will return list and tuple
...
actually creating the [42] value in the computer's memory and
storing a *reference* to it in the spam variable. When you copy the
value in [spam] and assign it to the variable
[cheese], you are actually copying the reference. Both the
[spam] and [cheese] variables refer to the
[42] value in the computer's memory. When you later change the
value in [spam] to [100], you're creating a new
[100] value and storing a reference to it in [spam].
This doesn't affect the value in [cheese]. Integers are
...
When you create the list [➊], you assign a reference to it in the
[spam] variable. But the next line [➋] copies only the
list reference in [spam] to [cheese], not the list
value itself. This means the values stored in [spam] and
[cheese] now both refer to the same list. There is only one
underlying list because the list itself was never actually copied. So
when you modify the first element of [cheese] [➌], you
are modifying the same list that [spam] refers to.
...
metaphor for variables, [Figure 4-4](#calibre_link-717) shows
...
[spam] is copied to [cheese]. Only a new reference
was created and stored in [cheese], not a new list. Note how
both references refer to the same list.


![image](../images/000132.jpg)


*Figure 4-5: [spam = cheese] copies the reference, not the
list.*

[]{#calibre_link-1734 {http:="" www.idpf.org="" 2007=""
ops}type="pagebreak"}When you alter the list that [cheese]
refers to, the list that [spam] refers to is also changed,
because both [cheese] and [spam] refer to the same
list. You can see this in [Figure 4-6](#calibre_link-719).
...
strings. We can use Python's [id()] function to understand
this. All values in Python have a unique identity that can be obtained
with the [id()] function. Enter the following into the
interactive shell:
...
mutable objects. The [append()] method doesn't create a new
...
[cheese] in the previous section) and the list value itself
changes, both variables are affected because they both refer to the same
list. The [append()], [extend()],
[remove()], [sort()], [reverse()], and
...
used to assign a new value to [spam]. Instead, it modifies the
...
[append('Hello')] method call inside the function affects
...
named [copy] that provides both the [copy()] and
[deepcopy()] functions. The first of these,
[copy.copy()], can be used to make a duplicate copy of a
...
modified when you assign [42] at index [1]. As you
can see in [Figure 4-7](#calibre_link-720), the reference ID
...
['#'] and [' '] strings that represent a living
...
cells. The [random.randint(0, 1)] call gives an even 50/50
...
cellular automata. On each step, we'll copy [nextCells] to
[currentCells], print [currentCells] on the screen,
...
The [%] mod operator performs a "wraparound." The left
neighbor of a cell in the leftmost column [0] would be [0 -
1] or [-1]. To wrap this around to the rightmost
column's index, [59], we calculate [(0 - 1) %
WIDTH]. Since [WIDTH] is [60], this
...
[Figure 4-8](#calibre_link-721), results in a pattern that
...
place---like, say, the [append()] or [remove()]
methods do on lists.
...
use [copy()] or [deepcopy()] if you want to make
...
[7](#calibre_link-728)!. What does
[bacon.append(99)] make the list value in [bacon]
look like?

[8](#calibre_link-729)!. What does
[bacon.remove('cat')] make the list value in
[bacon] look like?

[9](#calibre_link-730)!. What are the
operators for list concatenation and list replication?

[10](#calibre_link-731)!. What is the
difference between the [append()] and [insert()]
list methods?

[11](#calibre_link-732)!. What are two ways
to remove values from a list?

[12](#calibre_link-733)!. Name a few ways
that list values are similar to string values.

[13](#calibre_link-734)!. What is the
difference between lists and tuples?

[14](#calibre_link-735)!. How do you type
the tuple value that has just the integer value [42] in it?

[15](#calibre_link-736)!. How can you get
the tuple form of a list value? How can you get the list form of a tuple
value?

[16](#calibre_link-737)!. Variables that
"contain" list values don't actually contain lists directly. What do
they contain instead?

[17](#calibre_link-738)!. What is the
difference between [copy.copy()] and
[copy.deepcopy()]?
...
generates a list of randomly selected ['heads'] and
...
function call [random.randint(0, 1)] will return a
[0] value 50% of the time and a [1] value the other
50% of the time.
...
after each [print()] call.[]{#calibre_link-1741 {http:=""
