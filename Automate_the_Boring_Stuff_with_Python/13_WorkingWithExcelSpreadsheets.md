...
[openpyxl] module allows your Python programs to read and
...
[openpyxl] module can work on spreadsheets from these
...
[Figure 13-1](#calibre_link-1575) shows the
...
[]{#calibre_link-1575
.calibre6}![image](../images/000024.jpg)
...
13-1](#calibre_link-1576). (If you didn't download
...
manipulate it with the [openpyxl] module.

#### ***Opening Excel Documents with OpenPyXL*** 

Once you've imported the [openpyxl] module, you'll be able to
...
current working directory is by importing [os] and using
[os.getcwd()], and you can change the current working
...
a dictionary key. Finally, you can use the [active] attribute
of a [Workbook] object to get the workbook's active sheet. The
active sheet is the sheet that's on top when the workbook is opened in
Excel. Once you have the [Worksheet] object, you can get its
name from the [title] attribute.

#### ***Getting Cells from the Sheets*** 

Once you have a [Worksheet] object, you can access a
...
and [coordinate] attributes that provide location information
for the cell.
...
[datetime] data type is explained further in [Chapter
17](#calibre_link-530).
...
sheet's [cell()] method and passing integers for its
[row] and [column] keyword arguments. The first row
...
an odd row number. By passing [2] for the [range()]
function's "step" parameter, you can get cells from every second row (in
this case, all the odd-numbered rows). The [for] loop's
...
Note that the [max_column] attribute is an integer rather than
the letter that appears in Excel.

#### []***Converting Between Column Letters and Numbers*** 

To convert from letters to numbers, call the
[openpyxl.utils.column_index_from_string()] function. To
...
module, you can call [get_column_letter()] and pass it an
integer like 27 to figure out what the letter name of the 27th column
is. The function [column_index_string()] does the reverse: you
...
[Worksheet] object, and use a [Worksheet] attribute
like [max_column] to get an integer. Then, you can pass that
integer to [get_column_letter()].

#### ***Getting Rows and Columns from the Sheets*** 

You can slice [Worksheet] objects to get all the
[Cell] objects in a row, column, or rectangular area of the
...
Here, we specify that we want the [Cell] objects in the
rectangular area from A1 to C3, and we get a [Generator]
object containing the [Cell] objects in that area. To help us
visualize this [Generator] object, we can use
[tuple()] on it to display its [Cell] objects in a
tuple.
...
the [Cell] objects in one row of our desired area, from the
leftmost cell to the right. So overall, our slice of the sheet contains
all the [Cell] objects in the area from A1 to C3, starting
...
[➊]. Then, for each row, the nested [for] loop goes
through each cell in that row [➋].

To access the values of cells in a particular row or column, you can
also use a [Worksheet] object's [rows] and
[columns] attribute. These attributes must be converted to
lists with the [list()] function before you can use the square
...
Using the [rows] attribute on a [Worksheet] object
will give you a tuple of tuples. Each of these inner tuples represents a
row, and contains the [Cell] objects in that row. The
[columns] attribute also gives you a tuple of tuples, with
each of the inner tuples containing the [Cell] objects in a
particular []{#calibre_link-1063 {http:="" www.idpf.org="" 2007=""
ops}type="pagebreak"}column. For *example.xlsx*, since there are 7 rows
and 3 columns, [rows] gives us a tuple of 7 tuples (each
containing 3 [Cell] objects), and [columns] gives us
a tuple of 3 tuples (each containing 7 [Cell] objects).
...
you use [list(sheet.columns)[1]]. To get the tuple
containing the [Cell] objects in column A, you'd use
[list(sheet.columns)[0]]. Once you have a tuple representing
one row or column, you can loop through its [Cell] objects and
print their values.
...
6.  Use indexing or the [cell()] sheet method with
...
*[https://nostarch.com/automatestuff2/](https://nostarch.com/automatestuff2/)*.
Its contents look like [Figure 13-2](#calibre_link-1577).


[]{#calibre_link-1577
.calibre6}![image](../images/000116.jpg)
...
ops}type="pagebreak"}This code imports the [openpyxl] module,
as well as the [pprint] module that you'll use to print the
final county data [➊]. Then it opens the *censuspopdata.xlsx* file
[➋], gets the sheet with the census data [➌], and begins
iterating over its rows [➍].

Note that you've also created a variable named [countyData],
...
The data structure stored in [countyData] will be a dictionary
...
incrementing the value for [tracts] [➌] and increasing
the value for [pop] [➍] for the current county on each
...
Just as the [countyData] dictionary needs a dictionary as the
...
Since [setdefault()] will do nothing if the key already
exists, you can call it on every iteration of the [for] loop
without a problem.

#### ***Step 3: Write the Results to a File*** 

After the [for] loop has finished, the [countyData]
...
just use the [pprint.pformat()] function to write the
[countyData] dictionary value as a massive string to a file
...
Any time you modify the [Workbook] object or its sheets and
cells, the spreadsheet file will not be saved until you call the
[save()] workbook method. Enter the following into the
...
Remember to call the [save()] method to save the changes after
...
can use it just like a dictionary key on the [Worksheet]
...
spreadsheet from
*[https://nostarch.com/automatestuff2/](https://nostarch.com/automatestuff2/)*.
[Figure 13-3](#calibre_link-1578) shows what the spreadsheet
looks like.


[]{#calibre_link-1578
.calibre6}![image](../images/000062.jpg)
...
again, you'll need to update only the [PRICE_UPDATES]
...
[➊] [for rowNum in range(2, sheet.max_row):    # skip the first
row]
...
stored in the variable [produceName] [➋]. If
[produceName] exists as a key in the [PRICE_UPDATES]
dictionary [➌], then you know this is a row that must have its
...
saves the [Workbook] object to *updatedProduceSales.xlsx*
[➍]. It doesn't overwrite the old spreadsheet just in case there's
...
This allows you to type [Font()] instead of
[openpyxl.styles.Font()]. (See "[Importing
Modules](#calibre_link-125)" on [page
47](#calibre_link-827) to review this style of
[import] statement.)
...
[Font()]. [Table 13-2](#calibre_link-1579) shows
the possible keyword arguments for the [Font()] function.
...
ops}type="pagebreak"}You can call [Font()] to create a
[Font] object and store that [Font] object in a
variable. You then assign that variable to a [Cell] object's
...
we didn't specify a font name, so the [openpyxl] default,
Calibri, is used.
...
13-5](#calibre_link-1581).


[]{#calibre_link-1581
.calibre6}![image](../images/000100.jpg)
...
[Worksheet] objects have [row_dimensions] and
...
[RowDimension] objects and [column_dimensions]
contains [ColumnDimension] objects. In
[row_dimensions], you can access one of the objects using the
number of the row (in this case, 1 or 2). In
[column_dimensions], you can access one of the objects using
the letter of the column (in this case, A or B).

The *dimensions.xlsx* spreadsheet looks like [Figure
13-6](#calibre_link-1582).


[]{#calibre_link-1582
.calibre6}![image](../images/000046.jpg)


*Figure 13-6: Row 1 and column B set to larger heights and widths*

Once you have the [RowDimension] object, you can set its
height. Once you have the [ColumnDimension] object, you can
set its width. The row height can be set to an integer or float value
between [0] and [409]. This value represents the
...
or float value between [0] and [255]. This value
...
characters. Columns with widths of [0] or rows with heights of
...
13-7](#calibre_link-1583).


[]{#calibre_link-1583
.calibre6}![image](../images/000140.jpg)
...
known as *freeze panes*. In OpenPyXL, each [Worksheet] object
has a [freeze_panes] attribute that can be set to a
[Cell] object or a string of a cell's coordinates. Note that
...
1.  Create a [Reference] object from a rectangular selection
    of cells.
2.  Create a [Series] object by passing in the
    [Reference] object.
3.  Create a [Chart] object.
4.  Append the [Series] object to the [Chart]
    object.
5.  Add the [Chart] object to the [Worksheet]
    object, optionally specifying which cell should be the top-left
    corner of the chart.

The [Reference] object requires some explaining. You create
[Reference] objects by calling the
[openpyxl.chart.Reference()] function and passing three
arguments:
...
[Figure 13-9](#calibre_link-1586) shows some sample
coordinate arguments.


[]{#calibre_link-1586
.calibre6}![image](../images/000099.jpg)


*Figure 13-9: From left to right: [(1, 1), (10, 1)]; [(3, 2),
(6, 4)]; [(5, 3), (5, 3)]*
...
[]{#calibre_link-1587
.calibre6}![image](../images/000122.jpg)


*Figure 13-10: A spreadsheet with a chart added*

We've created a bar chart by calling
[openpyxl.chart.BarChart()]. You can also create line charts,
...
Equipped with the [openpyxl] module and some programming
...
For the following questions, imagine you have a [Workbook]
object in the variable [wb], a [Worksheet] object in
[sheet], a [Cell] object in [cell], a
[Comment] object in [comm], and an [Image]
object in [img].

[1](#calibre_link-1588). What does the
[openpyxl.load_workbook()] function return?

[2](#calibre_link-1589). What does the
[wb.sheetnames] workbook attribute contain?
...
[5](#calibre_link-1592). How would you
...
[8](#calibre_link-1595). What do the
[sheet.max_column] and [sheet.max_row] sheet
...
get the string name for column [14], what function would you
need to call?

[11](#calibre_link-1598). How can you
retrieve a tuple of all the [Cell] objects from A1 to F1?

[12](#calibre_link-1599). How would you
save the workbook to the filename *example.xlsx*?

[13](#calibre_link-1600). How do you set a
formula in a cell?

[14](#calibre_link-1601). If you want to
retrieve the result of a cell's formula instead of the cell's formula
itself, what must you do first?

[15](#calibre_link-1602). How would you
set the height of row 5 to 100?

[16](#calibre_link-1603). How would you
hide column C?

[17](#calibre_link-1604). What is a freeze
pane?

[18](#calibre_link-1605). What five
...
13-12](#calibre_link-1607).


[]{#calibre_link-1607
.calibre6}![image](../images/000013.jpg)
...
[for] loop to copy the first *N* lines. For the remaining
...
look something like [Figure 13-13](#calibre_link-1608).


[]{#calibre_link-1818 {http:="" www.idpf.org="" 2007=""
ops}type="pagebreak"}[]{#calibre_link-1608
.calibre6}![image](../images/000108.jpg)
...
structure could have [sheetData[x][y]] for the cell at
column [x] and row [y]. Then, when writing out the
new spreadsheet, use [sheetData[y][x]] for the cell at
column [x] and row [y].
...
Use the [readlines()] [File] object method to return
...
[readlines()] will be written to column 2, the next file to
column 3, and so on.
