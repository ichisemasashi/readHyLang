...
You can view the execution of this program at *[https://autbor.com/hellofunc/](https://autbor.com/hellofunc/)*.  The first line is a `def` statement [➊], which defines a function named `hello()`. The code in the block that follows the `def` statement [➋] is the body of the function.  This code is executed when the function is called, not when the function is first defined.
...
You can view the execution of this program at *[https://autbor.com/magic8ball/](https://autbor.com/magic8ball/)*.  When this program starts, Python first imports the `random` module [➊]. Then the `getAnswer()` function is defined [➋]. Because the function is being defined (and not called), the execution skips over the code in it. Next, the `random.randint()` function is called with two arguments: `1` and `9` [➍]. It evaluates to a random integer between `1` and `9` (including `1` and `9` themselves), and this value is stored in a variable named `r`.
...
You can view the execution of this program at *[https://autbor.com/readglobal/](https://autbor.com/readglobal/)*.  Since there is no parameter named `eggs` or any code that assigns `eggs` a value in the `spam()` function, when `eggs` is used in `spam()`, Python considers it a reference to the global variable `eggs`. This is why `42` is printed when the previous program is run.
...
You can view the execution of this program at *[https://autbor.com/localglobalsamename/](https://autbor.com/localglobalsamename/)*.  There are actually three different variables in this program, but confusingly they are all named `eggs`. The variables are as follows:
...
You can view the execution of this program at *[https://autbor.com/globalstatement/](https://autbor.com/globalstatement/)*.  Because `eggs` is declared `global` at the top of `spam()` [➊], when `eggs` is set to `'spam'` [➋], this assignment is done to the globally scoped `eggs`. No local `eggs` variable is created.
...
You can view the execution of this program at *[https://autbor.com/sameNameLocalGlobal/](https://autbor.com/sameNameLocalGlobal/)*.  In a function, a variable will either always be global or always be local. The code in a function can't use a local variable named `eggs` and then use the global `eggs` variable later in that same function.
...
You can view the execution of this program at *[https://autbor.com/zerodivide/](https://autbor.com/zerodivide/)*.  A `ZeroDivisionError` happens whenever you try to divide a number by zero. From the line number given in the error message, you know that the `return` statement in `spam()` is causing an error.
...
You can view the execution of this program at *[https://autbor.com/tryexceptzerodivide/](https://autbor.com/tryexceptzerodivide/)*.  Note that any errors that occur in function calls in a `try` block will also be caught. Consider the following program, which instead has the `spam()` calls in the `try` block:
...
You can view the execution of this program at *[https://autbor.com/spamintry/](https://autbor.com/spamintry/)*.  The reason `print(spam(1))` is never executed is because once the execution jumps to the code in the `except` clause, it does not return to the `try` clause. Instead, it just continues moving down the program as normal.
...
[1](#calibre_link-1215). Why are functions advantageous to have in your programs?

[2](#calibre_link-1216). When does the code in a function execute: when the function is defined or when the function is called?

[3](#calibre_link-1217). What statement creates a function?

[4](#calibre_link-1218). What is the difference between a function and a function call?

[5](#calibre_link-1219). How many global scopes are there in a Python program? How many local scopes?

[6](#calibre_link-1220). What happens to variables in a local scope when the function call returns?

[7](#calibre_link-1221). What is a return value? Can a return value be part of an expression?

[8](#calibre_link-1222). If a function does not have a return statement, what is the return value of a call to that function?

[9](#calibre_link-1223). How can you force a variable in a function to refer to the global variable?

[10](#calibre_link-1224). What is the data type of `None`?

[11](#calibre_link-1225). What does the `import areallyourpetsnamederic` statement do?

[12](#calibre_link-1226). If you had a function named `bacon()` in a module named `spam`, how would you call it after importing `spam`?

[13](#calibre_link-1227). How can you prevent a program from crashing when it gets an error?

[14](#calibre_link-1228). What goes in the `try` clause? What goes in the `except` clause?
