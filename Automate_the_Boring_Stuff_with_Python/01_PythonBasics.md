...
[F5], it will open the interactive shell, which will open as a
...
[+]), and they can always *evaluate* (that is, reduce) down to
...
value, [4]. A single value with no operators is also
...
1-1](#calibre_link-1652) lists all the math operators in
Python.
...
operators is similar to that of mathematics. The [\*\*]
operator is evaluated first; the [\*], [/],
[//], and [%] operators are evaluated next, from
left to right; and the [+] and [-] operators are
...
1-2](#calibre_link-1653). The values [-2] and
[30], for example, are said to be *integer* values. The
...
[42] is an integer, the value [42.0] would be a
...
([']) characters (as in ['Hello'] or ['Goodbye
cruel world!']) so Python knows where the string begins and
ends. You can even have a string with no characters in it,
[''], called a *blank string* or an *empty string*. Strings
...
values next to it. For example, [+] is the addition operator
when it operates on two integers or floating-point values. However, when
[+] is used on two string values, it joins the strings as the
...
try to use the [+] operator on a string and an integer value,
...
str] means that Python thought you were trying to concatenate
an integer to the string ['Alice']. Your code will have to
...
Your Program](#calibre_link-97)" on [page
13](#calibre_link-762) when we talk about the
[str()], [int()], and [float()]
functions.)

The [\*] operator multiplies two integer or floating-point
values. But when the [\*] operator is used on one string value
...
assignment statement [spam = 42], then a variable named
[spam] will have the integer value [42] stored in
it.

Think of a variable as a labeled box that a value is placed in, as in
[Figure 1-1](#calibre_link-1654).


[]{#calibre_link-1654
.calibre6}![image](../images/000026.jpg)


*Figure 1-1: [spam = 42] is like telling the program, "The
variable [spam] now has the integer value [42] in
it."*
...
time a value is stored in it [➊]. After that, you can use it in
expressions with other variables and values [➋]. When a variable
is assigned a new value [➌], the old value is forgotten, which is
why [spam] evaluated to [42] instead of
[40] at the end of the example. This is called *overwriting*
...
[spam] variable in this example stores ['Hello']
until you replace the string with ['Goodbye'].


[]{#calibre_link-1655
.calibre6}![image](../images/000097.jpg)
...
documentation) use generic variable names like [spam],
[eggs], and [bacon], which come from the Monty
...
1-3](#calibre_link-1656) has examples of legal variable
...
Variable names are case-sensitive, meaning that [spam],
[SPAM], [Spam], and [sPaM] are four
different variables. Though [Spam] is a valid variable you can
...
that is, variables [lookLikeThis] instead of
[looking_like_this]. Some experienced programmers may point
...
ops}type="pagebreak"}as soon as you press [ENTER]. The file
