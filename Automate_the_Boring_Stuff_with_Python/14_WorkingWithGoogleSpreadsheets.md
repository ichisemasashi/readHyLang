...
*[https://ezsheets.readthedocs.io/](https://ezsheets.readthedocs.io/)*.
...
www.idpf.org="" 2007=""
ops}type="pagebreak"}[google-auth-oauthlib] modules. These
...
-   *[https://console.developers.google.com/apis/library/sheets.googleapis.com/](https://console.developers.google.com/apis/library/sheets.googleapis.com/)*
-   *[https://console.developers.google.com/apis/library/drive.googleapis.com/](https://console.developers.google.com/apis/library/drive.googleapis.com/)*
...
[Figure 14-1](#calibre_link-1230). You'll need to log in to
your Google account to view this page.


![image](../images/000148.jpg)
...
[Figure 14-3](#calibre_link-1232).


[]{#calibre_link-1232
.calibre6}![image](../images/000036.jpg)
...
[Figure 14-3](#calibre_link-1232). Next, for Application
...
values. [Figure 14-4](#calibre_link-1233) shows a spreadsheet
...
.calibre6}![image](../images/000127.jpg)


*Figure 14-4: A spreadsheet titled "Education Data" with three sheets*

While most of your work will involve modifying the [Sheet]
objects, you can also modify [Spreadsheet] objects, as you'll
see in the next section.

#### ***Creating, Uploading, and Listing Spreadsheets*** 

You can make a new [Spreadsheet] object from an existing
spreadsheet, a blank spreadsheet, or an uploaded spreadsheet. To make a
[Spreadsheet] object from an existing Google Sheets
...
spreadsheet featured in [Figure 14-4](#calibre_link-1233) is
located at the URL
*https://docs.google.com/spreadsheets/d/1J-Jx6Ne2K_vqI9J2SO-TAXOFbxx_9tUjwnkPC22LjeU/edit#gid=151537240/*,
so its ID is [1J-Jx6Ne2K_vqI9J2SO-TAXOFbxx_9tUjwnkPC22LjeU].
...
[https://sheets.google.com/](https://sheets.google.com/) *to
...
[Spreadsheet] object of an existing spreadsheet by passing the
...
[ezsheets.createSpreadsheet()] function and pass it a string
...
[ezsheets.upload()]. Enter the following into the interactive
...
The [listSpreadsheets()] function returns a dictionary where
the keys are spreadsheet IDs and the values are the titles of each
spreadsheet.

Once you've obtained a [Spreadsheet] object, you can use its
...
[Spreadsheet] object has the following attributes for
manipulating the spreadsheet itself: [title],
...
your script can update the [Spreadsheet] object to match the
online data by calling the [refresh()] method:

>>> [ss.refresh()]

This will refresh not only the [Spreadsheet] object's
attributes but also the data in the [Sheet] objects it
contains. The changes you make to the [Spreadsheet] object
...
sheets, you'll need to change the [Sheet] object's
[index] attribute to [0]. See "[Creating and
Deleting Sheets](#calibre_link-468)" on [page
341](#calibre_link-1234) for information on how to do this.
...
folder at
*[https://drive.google.com/drive/trash](https://drive.google.com/drive/trash)*.
To permanently delete your spreadsheet, pass [True] for the
[permanent] keyword argument:
...
A [Spreadsheet] object will have one or more [Sheet]
objects. The [Sheet] objects represent the rows and columns of
data in each sheet. You can access these sheets using the square
brackets operator and an integer index. The [Spreadsheet]
object's [sheets] attribute holds a tuple of [Sheet]
...
You can also obtain a [Sheet] object with the square brackets
operator and a string of the sheet's name. The [Spreadsheet]
object's [sheetTitles] attribute holds a tuple of all the
...
Once you have a [Sheet] object, you can read data from and
write data to it using the [Sheet] object's methods, as
...
.calibre6}![image](../images/000074.jpg)
...
data in the [Sheet] object, call its [refresh()]
method:

>>> [sheet.refresh()]

All of the data in the [Sheet] object is loaded when the
[Spreadsheet] object is first loaded, so the data is read
...
not 0. You can convert the ['A2'] string-style address to
the [(column, row)] tuple-style address (and vice versa) with
the [convertAddress()] function. The
[getColumnLetterOf()] and [getColumnNumberOf()]
...
row)] tuple-style addresses are convenient if you're looping
over a range of addresses and need a numeric form for the column. The
[convertAddress()], [getColumnLetterOf()], and
[getColumnNumberOf()] functions are helpful when you need to
...
[getColumn()], [getRow()],
[updateColumn()], and [updateRow()] methods will,
...
You can pass [getColumn()] either a column number or letter to
tell it to retrieve a specific column's data. The previous example shows
that [getColumn(1)] and [getColumn('A')] return
the same list.

The [updateRow()] and [updateColumn()] functions
...
[updateRow()] call replaces it with data about pumpkin. Call
[sheet.getRow(3)] again to view the new values in the third
row.
...
To get all of the rows at once, call the [getRows()] method to
...
of some of the rows. Then you pass it to the [updateRows()]
...
[updateRows()] the list of lists returned from
[getRows()], amended with the changes made to rows 1 and 10.
...
count of [6], but we have only 4 columns of data. You can read
the number of rows and columns in a sheet with the [rowCount]
and [columnCount] attributes. Then by setting these values,
...
14-6](#calibre_link-1237).


[]{#calibre_link-1237
.calibre6}![image](../images/000021.jpg)
...
[createSheet()] method, to which you pass a string to use as
...
Every [Spreadsheet] object has an ordered list of the
[Sheet] objects it contains, and you can use this list to
reorder the sheets (as shown in the previous section) or copy them to
other spreadsheets. To copy a [Sheet] object to another
[Spreadsheet] object, call the [copyTo()] method.
...
previous example) already had a sheet named [Sheet1], the
copied sheet will be named [Copy of Sheet1]. Copied sheets
appear at the end of the list of the destination spreadsheet's sheets.
If you wish, you can change their [index] attribute to reorder
...
[googleapiclient.errors.HttpError] "Quota exceeded for quota
...
*[https://console.developers.google.com/quotas/](https://console.developers.google.com/quotas/)*
to learn about paying for increased usage. If you'd rather just deal
with the [HttpError] exceptions yourself, you can set
[ezsheets.IGNORE_QUOTA] to [True], and EZSheet's
...
as [Spreadsheet] objects, each of which contains an ordered
list of [Sheet] objects. Each sheet has columns and rows of
...
[Spreadsheet] object and a new [Sheet] object,
respectively?

[10](#calibre_link-1248). What will happen
...
*[https://goo.gl/forms/QZsq5sC2Qe4fYO592/](https://goo.gl/forms/QZsq5sC2Qe4fYO592/)*.
...
[upload()]. Once the spreadsheet has uploaded to Google
Sheets, download it using [downloadAsExcel()],
[downloadAsODS()], and other such functions to create a copy
...
*[https://docs.google.com/spreadsheets/d/1jDZEdvSIh4TmZxccyy0ZXrH-ELlrwq8_YYiZrEOB4jg/edit?usp=sharing/](https://docs.google.com/spreadsheets/d/1jDZEdvSIh4TmZxccyy0ZXrH-ELlrwq8_YYiZrEOB4jg/edit?usp=sharing/)*.
